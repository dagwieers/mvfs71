...
)
{
    ...
    if ((mnp->mn_vob.attr.fstat.mtime.tv_sec != vstat->fstat.mtime.tv_sec) ||
        (mnp->mn_vob.attr.fstat.mtime.tv_usec != vstat->fstat.mtime.tv_usec))
    {
        flags |= MFS_DTMMOD;
    }
    /* Compare vob modified times for changes in VOB (not view) */

    if ((vstat->event_time.tv_sec != mnp->mn_vob.attr.event_time.tv_sec) ||
        (vstat->event_time.tv_usec != mnp->mn_vob.attr.event_time.tv_usec))
    {
        flags |= MFS_VOBMOD;
    }
...
)
{
    ...
    modflags = mvfs_ac_set_stat(mnp, vstat, FALSE, MVFS_CD2CRED(cd));
...
)
{
    ...
    if (vstatp) {
        if (lvut) {
            ...
            modflags = mvfs_ac_set_stat(mnp, vstatp, TRUE, MVFS_CD2CRED(cd));
        } else {
            mnp->mn_vob.lvut.tv_sec = mnp->mn_vob.lvut.tv_usec = 0;
            modflags = mvfs_ac_set_stat(mnp, vstatp, FALSE, MVFS_CD2CRED(cd));
        }
    } else {
